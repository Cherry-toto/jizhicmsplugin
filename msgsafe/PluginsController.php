...
class PluginsController extends Controller {
	...
	public  function install(){
		//下面是新增test表的SQL操作
		//检测是否已安装前台插件
		$filepath = APP_PATH.'Home/plugins/MessageSafeController.php';
		if(file_exists($filepath)){
			JsonReturn(array('code'=>1,'msg'=>'前台Home下面已存在相应的MessageSafe控制器！'));
		}
		if(M('hook')->find(['plugins_name'=>'msgsafe'])){
		...
		$w['hook_controller'] = 'MessageSafe';
		$w['hook_action'] = 'checksafe';
		$w['plugins_name'] = 'msgsafe';//插件文件夹
		$w['addtime'] = time();
		M('hook')->add($w);
		return true;
	...
	public function uninstall(){
		//下面是删除test表的SQL操作
		M('hook')->delete(['plugins_name'=>'msgsafe']);
		return true;
	}
