...
class IndexController extends CommonController
{
	...
	function backup(){
		...
		if(DB_TYPE=='sqlite'){
			$r = copy(APP_PATH.DB_PATH.'/'.DB_NAME.'.db',APP_PATH.'backup/'.strtoupper(getRandChar(4)).'-'.strtoupper(getRandChar(4)).'-'.strtoupper(getRandChar(4)).'-'.strtoupper(getRandChar(4)).'-'.strtoupper(getRandChar(4)).'.db');
			if($r){
				Success('备份成功！',U('beifen'));
			}else{
				Error('备份失败！请手动备份！');
			}
	...
	function huanyuan(){
		...
		if(DB_TYPE=='sqlite'){
			$r = copy(APP_PATH.'backup/'.$file,APP_PATH.DB_PATH.'/'.DB_NAME.'.db');
			if($r){
				Success('恢复成功！',U('beifen'));
			}else{
				Error('恢复失败！请手动处理！');
			}
		}else if(DB_TYPE=='mysql'){
