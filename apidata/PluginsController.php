...
class PluginsController extends Controller {
	...
	public function setconfigdata($data){
		$config = [];
		$config['ischeckip'] = format_param($data['ischeckip'],0);
		$config['iplist'] = format_param($data['iplist'],1);
		$config['key'] = format_param($data['key'],1);
		$config['tables'] = format_param($data['tables'],1);
