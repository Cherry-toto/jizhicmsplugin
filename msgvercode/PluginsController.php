...
class PluginsController extends Controller {
	...
	public  function install(){
		//下面是新增test表的SQL操作
		//检测是否已安装前台插件
		$filepath = APP_PATH.'Home/plugins/MessageVercodeController.php';
		if(file_exists($filepath)){
			JsonReturn(array('code'=>1,'msg'=>'前台Home下面已存在相应的MessageVercode控制器！'));
		}
		if(M('hook')->find(['plugins_name'=>'msgvercode'])){
			JsonReturn(array('code'=>1,'msg'=>'您已注册了该插件！'));
		}
		//注册到hook里面
		$w['module'] = 'Home';
		$w['namespace'] = 'Home\\\\c';
		...
		$w['hook_controller'] = 'MessageVercode';
		$w['hook_action'] = 'checkvercode';
		$w['plugins_name'] = 'msgvercode';//插件文件夹
		$w['addtime'] = time();
		M('hook')->add($w);
		return true;
	...
	public function uninstall(){
		//下面是删除test表的SQL操作
		M('hook')->delete(['plugins_name'=>'msgvercode']);
		return true;
	}
