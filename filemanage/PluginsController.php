...
class PluginsController extends Controller {
	...
	public function setconfigdata($data){
		...
		$w['path'] = format_param($data['path'],1);
		$w['password'] = format_param($data['password'],1);
		//检查目录是否存在
		$dir = APP_PATH.'Home/'.$w['path'];
		if(!is_dir($dir)){
			JsonReturn(['code'=>1,'msg'=>'该目录不存在！']);
		}
		...
		//更改密码文件
		//删除123456.txt
		$r = file_put_contents(APP_PATH.'A/exts/filemanage/fileManage/'.$w['password'].'.txt',1);
		if(!$r){
