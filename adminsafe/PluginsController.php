...
class PluginsController extends Controller {
	...
	public function uninstall(){
		//下面是删除test表的SQL操作
		M('hook')->delete(['plugins_name'=>'adminsafe']);
		return true;
	}
	...
	public function setconfigdata($data){
		...
		if(array_key_exists('reset',$data)){
			M('plugins')->update(['id'=>$data['id']],['config'=>'']);
			setCache('hook',null);//清空hook缓存
			JsonReturn(['code'=>0,'msg'=>'重置成功！']);
		}else{
			$_SESSION['adminsafe'] = format_param($data['password'],1);
			$_SESSION['msg'] = format_param($data['msg'],1);
